## Beta v0.4.3
- Вернул ког Currency (Анти-ддос был снят с сайта, и бот снова может его парсить без проблем)
- Добавлено чтение всех сообщение и автомут, если в сообщение встречаются несколько из заданных слов (вы можете отключить это закомментировав эвент on_message в core/events.py)
- Добавлен ког timeout (аналог мута в других ботах) с выводов причины таймаута в журнал аудита
- Добавлен файл для своих запросов дискорду core/discord_requests.py
- Функция преобразования секунд в строковое время (361 - 6 minutes, 10 seconds) пересена в scripts/times.py
- Функции преобразования строкового времени формата "1d", "5w" и т.п. перенесены в scripts/transforms.py
- Исправлен показ ембеда помощи в коге "giveaway". Ранее ембед не отображался из-за пропущенного await
- Исправлена работа основной команды в коге "giveaway". Ранее выдавало ошибку из-за допущеной ошибки в команде all()
